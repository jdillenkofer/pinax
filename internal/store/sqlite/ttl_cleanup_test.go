...
func TestDeleteExpiredItemsBatched(t *testing.T) {
	...
	for i := 0; i < 250; i++ {
		...
		if err := s.PutItem(ctx, tx, table.Name, "S|"+fmt.Sprintf("e#%03d", i), model.NoSortKey, item); err != nil {
			tx.Rollback()
			t.Fatal(err)
		}
	}
	...
	if err := s.PutItem(ctx, tx, table.Name, "S|valid", model.NoSortKey, validItem); err != nil {
		tx.Rollback()
		t.Fatal(err)
	}
	...
	deleted1, err := s.DeleteExpiredItems(ctx, tx, table.Name, 1000, 100)
	if err != nil {
		t.Fatal(err)
	}
	deleted2, err := s.DeleteExpiredItems(ctx, tx, table.Name, 1000, 100)
	if err != nil {
		t.Fatal(err)
	}
	deleted3, err := s.DeleteExpiredItems(ctx, tx, table.Name, 1000, 100)
	if err != nil {
		t.Fatal(err)
	}
	...
	valid, err := s.GetItem(ctx, tx, table.Name, "S|valid", model.NoSortKey)
	if err != nil {
		t.Fatal(err)
	}
	if valid == nil {
